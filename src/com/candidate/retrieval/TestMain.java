	...
	public static void main(String[] args) throws IOException, ParseException, JWNLException {
		...
		if(args.length == 2)
		{
			...
			for(File files:filelist){
				...
				String text = DetectPlagiarism.readFileAsString(files);
				
				/**
				 * File top store the search results
				 */
				File newFile = new File(dest+files.getName()+".json");
				String fileid = files.getName().split("suspicious-document")[1].split("\\.")[0];
				String searchResult;
				...
				for(String sent:paragraphs){
					searchResult = ExecFlow.systemFlow(sent,tk,pos,stoptag,doc_wordtags,fileid,false);
